...
const AdminAbout = () => {
    ...
    const saveVideo = () => {
        ...
        setLoad(true)
        if (somos.length == 0){
            setErrors(['Digite alguma coisa no "Quem somos".'])
            setLoad(false)
        }else if (somos == aboutContent?.somos && !videoPreview){
    ...
    const saveMission = () => {
        ...
        setMessages([])
        setLoad(true)
        if (mission.length == 0 || visao.length == 0){
            setErrors(['Preencha todos os campos!']);
            setLoad(false)
        }else if (mission == aboutContent?.missao && visao == aboutContent?.visao) {
            setErrors(['Nenhuma alteração feita!'])
            setLoad(false)
        }else {
            axios.post(`${baseURL}/api/editAboutContent/any/${1}`, {
    ...
    const saveValue = () => {
        ...
        if (valueName.length == 0 || valueDescription.length == 0){
            setErrors(['Preencha todos os campos!'])
        }else if ([...aboutContent.valores].some((item) => item.nome == valueName)){
            setErrors(['Já tem valor com esse nome. Adicione um outro nome!'])
        }else {
    ...
    const editValue = (item) => {
        ...
        if (valueName.length == 0 || valueDescription.length == 0){
            ...
            .then(res => {
                let obj = [...res.data][0]
                let valores = [...obj.valores]
                let newValues = valores.map((t) => {
                    if (t.id == item.id){
                        return {
                            id: item.id,
                            nome: valueName,
    ...
    useEffect(()=> {
        axios.get(`${baseURL}/api/aboutContents`)
        .then(res => {
            res.data[0] && setSomos([...res.data][0].somos)
            res.data[0] && setMission([...res.data][0].missao)
            res.data[0] && setVisao([...res.data][0].visao)
    ...
    return (
        <div className="dashboardContainer" id='adminAbout' >
                        ...
                        <Dialog>
                            ...
                            <div className='sectionForms'>
                                <div className='form'>
                                    ...
                                    <textarea value={somos} onChange={(e)=>setSomos(e.target.value)} placeholder='Escreva quem somos...' className='loginInput' style={{height: 100}}/>
                                </div>
                                <div className="form">
            ...
            {aboutContent && <video autoPlay loop muted src={aboutContent.video}/>}
                ...
                <div className='header'>
                        <HeaderLabel title='Missão e Visão'/>
                        <Dialog>
                            <DialogTrigger>
                                <Edit title='Editar'/>
                            </DialogTrigger>
                            ...
                            <DialogTitle>Editar</DialogTitle>
                            <DialogDescription>
                                Editar missão e visão
                            </DialogDescription>
                            </DialogHeader>
                            <div className='sectionForms'>
                                <div className='form'>
                                    <div className='label'>Missão</div>
                                    <textarea placeholder='Escreva a missão...' 
                                    value={mission} onChange={(e) => setMission(e.target.value)}
                                    className='loginInput' style={{height: 100}}/>
                                </div>
                                <div className='form'>
                                <div className='label'>Visão</div>
                                    <textarea placeholder='Escreva a visão...' 
                ...
                {/* <section className='section' style={{display: 'flex', flexDirection: 'column', gap: 10, border: 'none'}}>
                <section className='section' style={{ }}>
                    <div className='header'>
                        <HeaderLabel title='Missão e Valores'/>
                        <Edit title='Editar'/>
            ...
            <div className='first' style={{marginTop: 10, paddingBottom: 20}}>
            <section className='section'>
                    <div className='header'>
                        <HeaderLabel title='Valores'/>
<Dialog>
                            ...
                            <div className='sectionForms'>
                                <div className='form'>
                                    <div className='label'>Nome do valor</div>
                                    <input placeholder='Escreva o nome do valor...' 
                                    value={valueName} onChange={(e) => setValueName(e.target.value)}
                                    className='loginInput'/>
                                </div>
                                <div className='form'>
                                <div className='label'>Descrição do valor</div>
  ...
  <path d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4zM2.5 3h11V2h-11z"/>
                    ...
                    :<div style={{fontSize: 14, textAlign: 'center'}}>Nenhum valor registrado. Adicione um.</div>
                    }
